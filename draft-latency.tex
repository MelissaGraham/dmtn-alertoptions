...
A discussion of the science drivers regarding the alert packet distribution latency.
}
...
This document considers the science motivation and impacts of the alert distribution timescale, also called the latency.
...
The 5 full streams aspect is based in part on estimates of the alert stream data rate and the bandwidth allocated to alert distribution in the LSST data facility \citedsp{DMTN-102}.
The 60 second aspect is scientifically motivated by a desire to explore the parameter space for very short timescale transients and variables which might last only a few minutes \citedsp{LPM-17}.

Increasing the alert latency could enable the stream to be delivered to more brokers, thereby increasing the amount of alerts-related science done by the community.
Or, it might allow human and computational resources currently dedicated to building a DMS that can deliver a 1 minute latency to be repurposed. 
In case it ever becomes technically necessary to change the alert latency, this document collects the science drivers for short timescale alert distribution.
