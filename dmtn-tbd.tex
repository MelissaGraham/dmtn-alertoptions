...
%In \S~\ref{sec:packets} we consider the potential science impact of options for the alert contents that could reduce packet size, and a few small additions that would assist with source classification. In \S~\ref{sec:latency} we consider the various science drivers for an alert distribution latency of 1 minute, and the potential scientific impact of a relaxed (e.g., 5 minute) latency. In \S~\ref{sec:prefilter} and \ref{sec:graceful} we consider the potential effects on scientific productivity regarding options to provide brokers with pre-filtered streams and delayed alert packets.
